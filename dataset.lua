...
function dataset:__init(...)
   ...
   --count how many images are there in the dataset while creating the cache
   --cannot do anything with this when loading from test cache
...
function dataset:get(i1, i2)
   ...
   --create a gpu batch here
   for i=1,quantity do
      -- load the sample ; impath is single image loaded either from cache or whatever
      local imgpath = ffi.string(torch.data(self.imagePaths[indices[i]])) ; --single img path 
      local id = ffi.string(torch.data(self.ids[indices[i]])) --single corr. img id
       --return false if it is not an image ; true if it is an image
       if pcall( isImage,imgpath,id ) then
          local out = self:sampleHook(imgpath) ; --out is a single image tensor of corr. img path
   ...
   --make the batch into a single tensor from a table
